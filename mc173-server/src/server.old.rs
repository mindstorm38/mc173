...
use crate::proto::{InPacket, OutPacket, Network, NetworkEvent, NetworkClient,
...
impl Server {
    ...
    pub fn bind(addr: SocketAddr) -> io::Result<Self> {
        let packet_server = Network::bind(addr)?;
        Ok(Self {
            overworld: ServerWorld::new(&packet_server, new_overworld()),
            players: Players {
                players: Vec::new(),
    ...
    pub fn tick(&mut self) -> AnyResult<()> {
        ...
        while let Some(event) = self.packet_server.poll()? {
            match event {
                ...
                NetworkEvent::Received { client, packet } => {
        ...
        // thread!
        let mut events: Vec<TcpEvent<InPacket>> = Vec::new();
        ...
        if time % 20 == 0 {
            let time_packet = OutPacket::UpdateTime(UpdateTimePacket { time });
        ...
        for event in self.resources.overworld_events.drain(..) {
            ...
            match event {
                ...
                Event::BlockChange { pos, new_block: new_id, new_metadata, .. } => {
                    ...
                    let block_change_packet = OutPacket::BlockChange(BlockChangePacket {
                        x: pos.x,
                        y: pos.y as i8,
                        z: pos.z,
                        block: new_id,
...
struct Players {
    ...
    players_client_map: HashMap<NetworkClient, usize>,
}
...
impl Player {
    ...
    fn handle_packet(&mut self, res: &mut Resources, packet: InPacket) -> io::Result<()> {
        match packet {
    ...
    fn handle_handshake(&mut self, res: &mut Resources, _username: String) -> io::Result<()> {
        ...
        res.tcp_server.send(self.client_id, &OutPacket::Handshake(ClientHandshakePacket {
            server: "-".to_string()
        }))
    ...
    fn handle_login(&mut self, res: &mut Resources, protocol_version: i32, username: String) -> io::Result<()> {
        ...
        if protocol_version != 14 {
            res.tcp_server.send(self.client_id, &OutPacket::Disconnect(DisconnectPacket {
        ...
        res.tcp_server.send(self.client_id, &OutPacket::Login(ClientLoginPacket {
            entity_id,
            random_seed: 0,
            dimension: 0,
        }))?;

        res.tcp_server.send(self.client_id, &OutPacket::SpawnPosition(SpawnPositionPacket {
            pos: res.overworld_dim.spawn_pos(),
        }))?;

        res.tcp_server.send(self.client_id, &OutPacket::UpdateTime(UpdateTimePacket {
            time: res.overworld_dim.time(),
        }))?;

        res.broadcast_packets.push(OutPacket::Chat(ChatPacket {
    ...
    fn handle_move(&mut self, 
        ...
        if !playing.initialized {
            ...
            res.tcp_server.send(self.client_id, &OutPacket::PositionLook(PositionLookPacket {
                pos: self.last_pos,
                look: self.last_look,
        ...
        let mut map_chunk_packet = OutPacket::ChunkData(ChunkDataPacket {
            x: 0, y: 0, z: 0, 
            ...
            for cz in -2..2 {
                ...
                if let Some(chunk) = res.overworld_dim.chunk(cx, cz) {
                    if playing.sent_chunks.insert((cx, cz)) {
                        ...
                        res.tcp_server.send(self.client_id, &OutPacket::ChunkState(ChunkStatePacket {
                            cx, cz, init: true
                        }))?;

                        if let OutPacket::ChunkData(packet) = &mut map_chunk_packet {
...
impl EntityTracker {
    ...
    fn tick(&mut self, server: &mut TcpServer) -> io::Result<()> {
        ...
        if let (false, Some(dx), Some(dy), Some(dz)) = (forced_position, dx, dy, dz) {
            ...
            if send_pos && send_look {
                move_packet = Some(OutPacket::EntityMoveAndLook(EntityMoveAndLookPacket {
                ...
                move_packet = Some(OutPacket::EntityMove(EntityMovePacket {
                    entity_id: self.entity_id,
                    dx,
                    dy,
                    dz,
                ...
                move_packet = Some(OutPacket::EntityLook(EntityLookPacket {
                    entity_id: self.entity_id,
                    yaw: self.look.x as i8,
                    pitch: self.look.y as i8,
                }))
            ...
            move_packet = Some(OutPacket::EntityPositionAndLook(EntityPositionAndLookPacket {
                entity_id: self.entity_id,
                x: self.pos.x,
                y: self.pos.y,
                z: self.pos.z,
    ...
    fn spawn_player_entity(&mut self, player: &Player, world: &World, server: &mut TcpServer) -> io::Result<()> {
        ...
        if let Some(entity) = entity.downcast_ref::<PlayerEntity>() {
            server.send(player.client_id, &OutPacket::PlayerSpawn(PlayerSpawnPacket {
                entity_id: entity.id,
            ...
            server.send(player.client_id, &OutPacket::ItemSpawn(ItemSpawnPacket { 
                entity_id: entity.id, 
            ...
            server.send(player.client_id, &OutPacket::MobSpawn(MobSpawnPacket {
                entity_id: entity.id,
                kind: 90,
                x: self.sent_pos.x, 
                y: self.sent_pos.y, 
    ...
    fn kill_player_entity(&mut self, player: &Player, server: &mut TcpServer) -> io::Result<()> {
        server.send(player.client_id, &OutPacket::EntityKill(EntityKillPacket { 
            entity_id: self.entity_id
        }))
    }
