...
/// Extension trait for world that provides redstone power calculation.
pub trait WorldPower: World {

    /// Check if the given block position get any active power from surrounding faces.
    #[inline]
    fn has_active_power(&mut self, pos: IVec3) -> bool {
    ...
    #[inline]
    fn has_passive_power(&mut self, pos: IVec3) -> bool {
    ...
    #[inline]
    fn has_active_power_from(&mut self, pos: IVec3, face: Face) -> bool {
        self.get_active_power_from(pos, face) > 0
    }

    /// Return true if the given block's face has any passive power.
    #[inline]
    fn has_passive_power_from(&mut self, pos: IVec3, face: Face) -> bool {
        self.get_passive_power_from(pos, face) > 0
    }

    /// Get the active power produced by a block's face.
    fn get_active_power_from(&mut self, pos: IVec3, face: Face) -> u8 {
        let power = get_power_from(self, pos, face, true);
    ...
    fn get_passive_power_from(&mut self, pos: IVec3, face: Face) -> u8 {
        get_power_from(self, pos, face, true).level
    }

}

/// Standard implementation.
impl<W: World> WorldPower for W { }


/// Get the power produced by a block on a given face.
fn get_power_from(world: &mut impl World, pos: IVec3, face: Face, test_block: bool) -> Power {

    let Some((id, metadata)) = world.get_block(pos) else { return Power::OFF };

    match id {
        block::LEVER => get_lever_power_from(world, face, metadata),
        block::BUTTON => get_button_power_from(world, face, metadata),
        block::REPEATER_LIT => get_repeater_power_from(world, face, metadata),
        block::REDSTONE_TORCH_LIT => get_redstone_torch_power_from(world, face, metadata),
...
fn get_block_power_from(world: &mut impl World, pos: IVec3, face: Face) -> Power {
...
fn get_lever_power_from(world: &mut impl World, face: Face, metadata: u8) -> Power {
...
fn get_button_power_from(world: &mut impl World, face: Face, metadata: u8) -> Power {
...
fn get_repeater_power_from(world: &mut impl World, face: Face, metadata: u8) -> Power {
...
fn get_redstone_torch_power_from(world: &mut impl World, face: Face, metadata: u8) -> Power {
...
fn get_redstone_power_from(world: &mut impl World, pos: IVec3, face: Face, metadata: u8) -> Power {
    if face == Face::PosY || metadata == 0 {
        ...
        let opaque_above = world.get_block(pos + IVec3::Y)
...
/// given face.
fn is_linkable_from(world: &mut impl World, pos: IVec3, face: Face) -> bool {
    if let Some((id, metadata)) = world.get_block(pos) {
        match id {
            block::LEVER |
            block::BUTTON |
            block::DETECTOR_RAIL |
