...
use super::bound::{WorldBound, RayTraceKind};
...
/// Extension trait to world that provides explosion in the world.
pub trait WorldExplode: World {

    /// Make an explosion in the world at the given position and size. The explosion can
    /// optionally propagate flames around.
    fn explode(&mut self, center: DVec3, radius: f32, set_fire: bool, origin_id: Option<u32>) {
            ...
            for dy in 0..16 {
                for dz in 0..16 {
                    if dx == 0 || dx == 15 || dy == 0 || dy == 15 || dz == 0 || dz == 15 {
                        ...
                        let mut intensity = radius * (0.7 + self.get_rand_mut().next_float() * 0.6);
...
impl<W: World> WorldExplode for W { }
