...
use super::interact::WorldInteract;
use super::material::WorldMaterial;
use super::r#break::WorldBreak;
use super::power::WorldPower;


/// Extension trait for world that provides block notification.
pub trait WorldNotify: World {
    ...
    fn notify_block_unchecked(&mut self, pos: IVec3, id: u8, metadata: u8, origin_id: u8) {
        match id {
            block::REDSTONE if origin_id != block::REDSTONE => notify_redstone(self, pos),
            block::REPEATER |
            block::REPEATER_LIT => notify_repeater(self, pos, id, metadata),
            block::REDSTONE_TORCH |
            block::REDSTONE_TORCH_LIT => notify_redstone_torch(self, pos, id),
            block::DISPENSER => notify_dispenser(self, pos, origin_id),
            block::WATER_MOVING |
            block::LAVA_MOVING => notify_fluid(self, pos, id, metadata),
            block::WATER_STILL |
            block::LAVA_STILL => notify_fluid_still(self, pos, id, metadata),
            block::TRAPDOOR => notify_trapdoor(self, pos, metadata, origin_id),
            block::WOOD_DOOR |
            block::IRON_DOOR => notify_door(self, pos, id, metadata, origin_id),
            block::DANDELION |
            block::POPPY |
            block::SAPLING |
            block::TALL_GRASS => notify_flower(self, pos, &[block::GRASS, block::DIRT, block::FARMLAND]),
            block::DEAD_BUSH => notify_flower(self, pos, &[block::SAND]),
            block::WHEAT => notify_flower(self, pos, &[block::FARMLAND]),
            block::RED_MUSHROOM |
            block::BROWN_MUSHROOM => notify_mushroom(self, pos),
            block::CACTUS => notify_cactus(self, pos),
            block::SAND |
            block::GRAVEL => self.schedule_block_tick(pos, id, 3),
            block::FIRE => { notify_fire(self, pos); },
            block::PISTON |
            block::STICKY_PISTON => notify_piston(self, pos, id, metadata),
            block::PISTON_EXT => notify_piston_ext(self, pos, metadata, origin_id),
            block::NOTE_BLOCK => notify_note_block(self, pos, origin_id),
            _ => {}
        }
    }

    /// Notify a block change at given position.
    fn notify_change_unchecked(&mut self, pos: IVec3, from_id: u8, from_metadata: u8, to_id: u8, to_metadata: u8) {
        ...
        match to_id {
            ...
            block::REDSTONE => notify_redstone(self, pos),
            block::REPEATER |
            block::REPEATER_LIT => notify_repeater(self, pos, to_id, from_metadata),
            block::REDSTONE_TORCH |
            block::REDSTONE_TORCH_LIT => notify_redstone_torch(self, pos, to_id),
            block::SAND |
            block::GRAVEL => self.schedule_block_tick(pos, to_id, 3),
            block::CACTUS => notify_cactus(self, pos),
            block::FIRE => notify_fire_place(self, pos),
            block::PISTON |
            block::STICKY_PISTON => notify_piston(self, pos, to_id, to_metadata),
            _ => {}
        }
...
impl<W: World> WorldNotify for W { }


/// Notification of a moving fluid block.
fn notify_fluid(world: &mut impl World, pos: IVec3, id: u8, metadata: u8) {
    ...
    if id == block::LAVA_MOVING {
        ...
        for face in Face::HORIZONTAL {
            if let Some((block::WATER_MOVING | block::WATER_STILL, _)) = world.get_block(pos + face.delta()) {
                // If there is at least one water block around.
                if distance == 0 {
                    world.set_block_notify(pos, block::OBSIDIAN, 0);
                } else if distance <= 4 {
                    world.set_block_notify(pos, block::COBBLESTONE, 0);
                }
            }
        }
    }
...
fn notify_fluid_still(world: &mut impl World, pos: IVec3, id: u8, metadata: u8) {

    // Subtract 1 from id to go from still to moving.
    let moving_id = id - 1;

    notify_fluid(world, pos, moving_id, metadata);
    world.set_block_self_notify(pos, moving_id, metadata);

}

/// Notification of standard flower subclasses.
fn notify_flower(world: &mut impl World, pos: IVec3, stay_blocks: &[u8]) {
    if world.get_light(pos).max() >= 8 || false /* block can see sky */ {
        let (below_id, _) = world.get_block(pos - IVec3::Y).unwrap_or((0, 0));
...
fn notify_mushroom(world: &mut impl World, pos: IVec3) {
    if world.get_light(pos).max() >= 13 || !world.is_block_opaque_cube(pos - IVec3::Y) {
        world.break_block(pos);
    }
}

/// Notification of a cactus block. The block is broken if 
fn notify_cactus(world: &mut impl World, pos: IVec3) {
    for face in Face::HORIZONTAL {
    ...
    if !matches!(world.get_block(pos - IVec3::Y), Some((block::CACTUS | block::SAND, _))) {
        world.break_block(pos);
    }
}
...
fn notify_fire(world: &mut impl World, pos: IVec3) -> bool {

    for face in Face::ALL {
        if let Some((id, _)) = world.get_block(pos + face.delta()) {
...
fn notify_fire_place(world: &mut impl World, pos: IVec3) {
    
    if notify_fire(world, pos) {
        return;
    }

    // Check where there is obsidian around.
    let obsidians = Face::HORIZONTAL.into_iter()
        .filter(|face| world.is_block(pos + face.delta(), block::OBSIDIAN))
        .collect::<FaceSet>();
    ...
    if obsidians.contains_x() != obsidians.contains_z() {
        ...
        for dxz in -1..=2 {
            for dy in -1..=3 {
                if (dxz != -1 && dxz != 2) || (dy != -1 && dy != 3) {
                    ...
                    let Some((id, _)) = world.get_block(pos + factor * IVec3::new(dxz, dy, dxz)) else {
                        valid = false;
                        break;
                    };
        ...
        if valid {
            for dxz in 0..2 {
                for dy in 0..3 {
                    world.set_block_notify(pos + factor * IVec3::new(dxz, dy, dxz), block::PORTAL, 0);
                }
            }
            return;
        }
    ...
    world.schedule_block_tick(pos, block::FIRE, 40)

}

/// Notification of a redstone repeater block.
fn notify_repeater(world: &mut impl World, pos: IVec3, id: u8, metadata: u8) {
    ...
    if lit != back_powered {
        world.schedule_block_tick(pos, id, delay);
    }

}

/// Notification of a redstone repeater block.
fn notify_redstone_torch(world: &mut impl World, pos: IVec3, id: u8) {
    world.schedule_block_tick(pos, id, 2);
}

fn notify_dispenser(world: &mut impl World, pos: IVec3, origin_id: u8) {
    if is_redstone_block(origin_id) {
        // TODO: Also check above? See associated tick function.
        if world.has_passive_power(pos) {
            world.schedule_block_tick(pos, block::DISPENSER, 4);
        }
    }
}
...
fn notify_trapdoor(world: &mut impl World, pos: IVec3, mut metadata: u8, origin_id: u8) {
    ...
    if !world.is_block_opaque_cube(pos + face.delta()) {
        ...
        if is_redstone_block(origin_id) {
            let powered = world.has_passive_power(pos);
            if open != powered {
...
fn notify_door(world: &mut impl World, pos: IVec3, id: u8, mut metadata: u8, origin_id: u8) {

    if block::door::is_upper(metadata) {
        
        // If the block below is not another door,
        if let Some((below_id, below_metadata)) = world.get_block(pos - IVec3::Y) {
            if below_id == id {
                notify_door(world, pos - IVec3::Y, below_id, below_metadata, origin_id);
                return;
            }
        }
        ...
        if is_redstone_block(origin_id) {
            ...
            let mut powered = 
                world.has_passive_power_from(pos - IVec3::Y, Face::PosY) ||
                world.has_passive_power_from(pos + IVec3::Y * 2, Face::NegY);

            if !powered {
                for face in Face::ALL {
                    let face_pos = pos + face.delta();
                    powered = 
                        world.has_passive_power_from(face_pos, face.opposite()) || 
                        world.has_passive_power_from(face_pos + IVec3::Y, face.opposite());
                    if powered {
                        break;
                    }
                }
            ...
            if block::door::is_open(metadata) != powered {
                ...
                world.notify_block(pos - IVec3::Y, id);
                world.notify_block(pos + IVec3::Y * 2, id);
                for face in Face::ALL {
                    world.notify_block(pos + face.delta(), id);
                    world.notify_block(pos + face.delta() + IVec3::Y, id);
                }
...
fn notify_piston(world: &mut impl World, pos: IVec3, id: u8, metadata: u8) {
    ...
    let powered = Face::ALL.into_iter()
        ...
        .any(|face| world.has_passive_power_from(pos + face.delta(), face.opposite()));
    ...
    if powered != extended {
        ...
        if powered {
            ...
            loop {
                ...
                let Some((check_id, check_metadata)) = world.get_block(check_pos) else {
            ...
            if let Some((prev_id, prev_metadata)) = world.set_block(check_pos, block::AIR, 0) {
            ...
            for _ in 0..=push_count {
                ...
                world.set_block(move_pos, block::PISTON_MOVING, next_metadata).unwrap();
                world.set_block_entity(move_pos, BlockEntity::Piston(PistonBlockEntity {
                    block: next_id,
            ...
            if let Some(BlockEntity::Piston(piston)) = world.get_block_entity_mut(head_pos) {
                let (moving_id, moving_metadata) = (piston.block, piston.metadata);
                world.remove_block_entity(head_pos);
                if world.is_block(pos, block::PISTON_MOVING) {
                    world.set_block_notify(pos, moving_id, moving_metadata);
                }
            }

            // Now we replace the piston base by a moving piston block entity.
            world.set_block(pos, block::PISTON_MOVING, metadata);
            world.set_block_entity(pos, BlockEntity::Piston(PistonBlockEntity {
                block: id,
                metadata,
                face,
                progress: 0.0,
            ...
            if sticky {
                ...
                let Some((mut sticky_id, mut sticky_metadata)) = world.get_block(sticky_pos) else {
                ...
                if sticky_id == block::PISTON_MOVING {
                    if let Some(BlockEntity::Piston(piston)) = world.get_block_entity_mut(sticky_pos) {
                        if piston.extending && piston.face == face {
                            ...
                            world.remove_block_entity(head_pos);
                            if world.is_block(pos, block::PISTON_MOVING) {
                                world.set_block_notify(pos, sticky_id, sticky_metadata);
                            }
                        }
                    }
                }
                ...
                if sticky_drop || block::material::get_piston_policy(sticky_id, sticky_metadata) != PistonPolicy::PushPull {
                    world.set_block(head_pos, block::AIR, 0);
                } else {
                    world.set_block(sticky_pos, block::AIR, 0);
                    world.set_block(head_pos, block::PISTON_MOVING, sticky_metadata);
                    world.set_block_entity(head_pos, BlockEntity::Piston(PistonBlockEntity {
                        block: sticky_id,
        ...
        let head_pos = pos + delta;
        if !world.is_block(head_pos, block::PISTON_EXT) {
            world.break_block(pos);
        }
...
fn notify_piston_ext(world: &mut impl World, pos: IVec3, metadata: u8, origin_id: u8) {
    
    let Some(face) = block::piston::get_face(metadata) else { return };
    
    let base_pos = pos - face.delta();
    if let Some((base_id, base_metadata)) = world.get_block(base_pos) {
        if let block::PISTON | block::STICKY_PISTON = base_id {
            // Just forward the notification to the piston.
            world.notify_block_unchecked(base_pos, base_id, base_metadata, origin_id);
            return;
        }
    }
...
fn notify_note_block(world: &mut impl World, pos: IVec3, origin_id: u8) {
    ...
    if note_block.powered != powered {
        ...
        if powered {
            ...
            world.interact_block_unchecked(pos, block::NOTE_BLOCK, 0, true);
        }
    }
...
fn notify_redstone(world: &mut impl World, pos: IVec3) {
    ...
    while let Some((pending_pos, link_face)) = pending.pop() {
        ...
        node.opaque_above = world.get_block(pos + IVec3::Y)
            .map(|(above_id, _)| block::material::is_opaque_cube(above_id))
            .unwrap_or(true);
        node.opaque_below = world.get_block(pos - IVec3::Y)
        ...
        for face in FACES {
            ...
            if let Some((id, _)) = world.get_block(face_pos) {
                ...
                let face_power = world.get_active_power_from(face_pos, face.opposite());
                ...
                if block::material::get_material(id).is_opaque() {
                    ...
                    if !node.opaque_above {
                        ...
                        if let Some((block::REDSTONE, _)) = world.get_block(face_above_pos) {
                            node.links.insert(face);
                    ...
                    if let Some((block::REDSTONE, _)) = world.get_block(face_below_pos) {
                        node.links.insert(face);
    ...
    let mut inner_notify_at = move |pos: IVec3| {
        if notified.insert(pos) {
            world.notify_block(pos, block::REDSTONE);
        }
    };
